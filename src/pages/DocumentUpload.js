...
import { creatCase } from '../utils/requests';
...
export default function DocumentUpload({ userType }) {
	...
	const onSubmit = async () => {
		...
		n['white_label_id'] = JSON.parse(localStorage.getItem('wt_lbl')).id;

		const data = await creatCase(n, userDetails.userToken);
		console.log(data);
	};
	...
	return (
		<>
			...
			{showModal && <BankStatementModal showModal={showModal} onClose={() => setShowModal(false)} />}
		</>
	);
}
