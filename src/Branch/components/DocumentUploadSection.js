...
import { NC_STATUS_CODE, WHITELABEL_ENCRYPTION_API, SECRET } from '../../_config/app.config';
import { DOWNLOAD_CASE_DOCUMENTS, VIEW_CASE_DOCUMENTS_LIST } from '../../_config/branch.config';

const cooap = (data, type) => {
	return data?.directors.find(e => e?.type_name.toLowerCase() === type.toLowerCase() && e.id);
};
...
export default function DocumentUploadSection({
	...
	const FetchDocumentLists = async () => {
		try {
			...
			const encryptedWLId = encryptWhiteLabelReq?.data?.encrypted_whitelabel?.[0] || null;
	...
	const applicantDocs = documentList?.filter(docs => docs.uploadedBy === appplicantId);
	...
	return (
		<>
			<section className='flex flex-col gap-y-5 w-8/12'>
				<p className='text-blue-600 font-medium text-xl'>Applicant Documents Uploaded</p>
				<FileUpload
					accept=''
					upload={{
											...
											<section className='py-2 flex justify-evenly items-center w-full'>
												<section className='w-full'>
													<Button
														type='blue-light'
														onClick={() =>
															viewDocument(loanData?.id, j.uploadedBy, j.document_fd_key)
														}
													>
														{j.document_name}
													</Button>
			...
			{cooap(loanData, 'Co-Applicant')?.id && (
				<section className='flex flex-col space-y-5 w-8/12'>
					<p className='text-blue-600 font-medium text-xl'>Co-Applicant Documents Uploaded</p>
					<FileUpload
						accept=''
						upload={{
							url: DOCS_UPLOAD_URL_LOAN({
