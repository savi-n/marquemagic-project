...
export default function CheckApplication(props) {
  ...
  useEffect(() => {
    ...
    getLoanDetails(id).then((loanDetails) => {
      ...
      if (loanDetails) {
        verification(loanDetails?.business_id?.id, clientToken).then(
          (respo) => {
        ...
        if (loanDetails?.collateralData) {
          if (loanDetails.collateralData?.modified_collateral) {
            ...
            loanDetails.collateralData?.initial_collateral &&
            loanDetails.collateralData?.initial_collateral.length > 0
          ) {
  ...
  return (
    <main>
      ...
      <div
        onClick={() => dispatch(branchAction.setResetLoanAction())}
