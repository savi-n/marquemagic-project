...
export const getNCStatus = () => {
	axios.get('http://3.0.103.80:1337/case_nc_status', { headers: { Authorization: `${token}` } }).then(res => {
		return res;
	});
};
...
export const getCase = async order => {
	const g = await axios.post(
		`http://3.0.103.80:1337/branch/viewLoan`,
		{ ncStatus: `${order}` },
		{
...
export const getUsersList = async () => {
	const g = await axios.get(`http://3.0.103.80:1337/branch/getUserList`, {
		headers: { Authorization: `${token}` },
...
export const reassignLoan = async (loanId, reAssignTo, comments, recommendation) => {
	const g = await axios.post(
		`http://3.0.103.80:1337/branch/reAssignLoans`,
...
export const getLoanDetails = async loanId => {
	const g = await axios.post(
		`http://3.0.103.80:1337/cub/getLoanDeatils`,
		{ loanId },
		{
			headers: { Authorization: `${token}` }
		}
