...
export const getBankList = async (type, linkRequired, isEncryption) => {
  ...
  const data = await axios.post(
    "http://40.80.80.135:1337/sails-exp/ClientVerify",
    body
  );
  const generatedLink = await axios.post(
    "http://40.80.80.135:1337/generateLink",
    null,
    {
...
export const verifyOtp = async (paramsData) => {
  const data = await axios.post(
    "http://3.108.54.252:1337/cub/verifyOtp",
    null,
    {
      params: { ...paramsData },
    }
