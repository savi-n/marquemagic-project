...
const PanUpload = props => {
															...
															// 	null;
															const panExtractionData =
																uploadedFile?.panExtractionData;
															...
															const onProceedUdyodAadhar = async udyogAadharNumber => {
																try {
																	...
																	const VerifyUdyog = await axios.get(
																		`${
																			API.ENDPOINT_BANK
																		}/get/udyog?uan=${udyogAadharNumber}`,
																		{
																			headers: {
															...
															const onCompanySelect = async cinNumber => {
																...
																setLoading(true);
																await cinNumberFetch(
																	cinNumber
																);
															};
															const onProceedPanConfirm = async () => {
																try {
																	const panErrorMessage = isInvalidPan(
																		confirmPanNumber
																	);
																	if (
																		panErrorMessage
																	...
																	const gstinData = await gstinFetch(
																		confirmPanNumber
																	);
																	if (!gstinData) {
																		setIsUdyogModalOpen(
																	...
																	if (first_name) {
																		onChangeFormStateField(
																			{
																				name:
																					CONST_BASIC_DETAILS.FIRST_NAME_FIELD_NAME,
																				value:
																					first_name ||
																					'',
																			}
																	...
																	if (last_name) {
																		onChangeFormStateField(
																			{
																				name:
																					CONST_BASIC_DETAILS.LAST_NAME_FIELD_NAME,
																				value:
																					last_name ||
																					'',
																			}
																	...
																	if (
																		panExtractionData?.father_name
																	) {
																		onChangeFormStateField(
																			{
																				name:
																					CONST_BASIC_DETAILS.FATHER_NAME_FIELD_NAME,
																				value:
																					panExtractionData?.father_name ||
																					'',
																			}
																	...
																	if (
																		...
																		onChangeFormStateField(
																			{
																				name:
																					CONST_BASIC_DETAILS.DOB_FIELD_NAME,
																				value:
																					DOB || '',
																			}
																		);
																		...
																		onChangeFormStateField(
																			{
																				...
																				value:
																					moment(
																						companyRocData?.BusinessVintage
																					).format(
																						'YYYY-MM-DD'
																					) || '',
																			}
															...
															const handleExtractionPan = async file => {
																...
																try {
																	...
																	const panExtractionApiRes = await getKYCData(
																		formData,
																		clientToken
																	);
																	const panExtractionStatus =
																		panExtractionApiRes
																			?.data
																			?.status || '';
																	const panExtractionMsg =
																		panExtractionApiRes
																			?.data
																			?.message ||
																		'';
																	const panForensicRes =
																		panExtractionApiRes
																			?.data
																			?.forensicData ||
																		{};
																	const panForensicFlag =
																		panForensicRes?.flag?.toLowerCase() ||
																		'';
																	const panForensicFlagMsg =
																		panForensicRes?.flag_message ||
																		'';
																	...
																	newFileData = {
																		...
																		type:
																			CONST_SECTIONS.EXTRACTION_KEY_PAN,
																		req_type:
																			CONST_SECTIONS.EXTRACTION_KEY_PAN, // requires for mapping with JSON
																		requestId:
																			panExtractionApiRes
																				?.data
																				?.request_id,
																	...
																	console.error(
																		'error-pan-verification-handleExtractionPan-',
																		error
																	);
																	setIsPanConfirmModalOpen(
																		true
															...
															const {
																...
																onDrop: async acceptedFiles => {
																	try {
																		...
																		// 	)
																		// );
																		await handleExtractionPan(
																			acceptedFiles[0]
																		);
																	} catch (error) {
																		console.error(
																			'error-ProfileFileUpload-onDrop-',
																			error
																		);
																	} finally {
																		setLoading(
																		...
																		<section className='p-4 flex flex-col gap-y-8'>
																			...
																			<UI.ConfirmPanWrapper>
																				...
																				<UI.FieldWrapperPanVerify>
																					<InputField
																						name={
																							CONST_BASIC_DETAILS.PAN_NUMBER_CONFIRM_FIELD_NAME
																						}
																						value={
																							confirmPanNumber
																						}
																							...
																							<UI.IconDelete
																								...
																								onClick={e => {
																									...
																									onChangeFormStateField(
																										{
																											name:
																												CONST_BASIC_DETAILS.PAN_UPLOAD_FIELD_NAME,
																											value:
																												'',
																										}
																									);
