...
const AddressDetails = props => {
				...
				const [
					verifyOtpResponseTemp,
					setVerifyOtpResponseTemp,
				] = useState(null);
				const selectedVerifyOtp =
					verifyOtpResponseTemp ||
					selectedApplicant?.api?.verifyOtp ||
					null;
				...
				const onProceed = async () => {
					...
					try {
						if (
							...
							return addToast({
								message:
									'Please enter valid pincode to get city and state',
								type: 'error',
							});
						}
								...
								if (!businessAddressIdAid1) {
									...
									if (!isPermanentSelectedAddressProofTypeOthers) {
										...
										permanentCacheDocumentsTemp.map(doc => {
											if (!!doc?.extractionRes)
												isFetchAddressPressed = true;
											return null;
										});
						...
						const newLoanAddressDetails = [
							{
								...
								residential_type:
									formState?.values?.present_property_type || '',
							...
							},
							{
								...
								residential_type:
									formState?.values?.permanent_property_type || '',
								residential_stability:
						...
						if (otherdocs.length > 0) {
							...
							const callLoanDocUpload = async idx => {
								...
								let result = await axios.post(
									`${
										API.API_END_POINT
									}/loanDocumentUpload?userId=${businessUserId}`,
									formData
								);
							...
							otherDocsBorrowerApi?.map(doc => {
								...
								newOtherUploadedDocumentsTemp.push({
									...
									classification_type:
										doc?.isTagged?.classification_type,
									classification_sub_type:
										doc?.isTagged?.classification_sub_type,
									...
									original_doc_name:
										formState?.values?.[
											`${doc?.prefix}${
												CONST.OTHERS_DOC_NAME_FIELD_NAME
											}`
										],
				...
				const prefilledEditOrViewLoanValues = field => {
					const preData = {
						...
						permanent_property_type:
							selectedApplicant?.permanent_residential_type,
				...
				const prefilledValues = field => {
					try {
								...
								if (
									isTestMode &&
									CONST.initialFormState?.[field?.name]
								) {
						...
						{selectedSection?.sub_sections?.map(
							(sub_section, subSectionIndex) => {
								...
								const selectedAddressProofTypeOption = sub_section.fields
									.filter(
										field =>
											field.name === selectedAddressProofFieldName
									)?.[0]
									?.options?.filter(
								...
								const selectedDocTypeId =
									selectedAddressProofTypeOption?.doc_type?.[
										selectedIncomeType
									];
								...
								if (selectedAddressProofId) {
									const isFrontTagged =
										cacheDocumentsTemp?.filter(
											f =>
												f?.isTagged?.id ===
												selectedDocumentTypes?.[0]?.id
										).length > 0;
									const isBackTagged =
										cacheDocumentsTemp?.filter(
											f =>
												f?.isTagged?.id ===
												selectedDocumentTypes?.[1]?.id
										).length > 0;
									const isFrontBackTagged =
										cacheDocumentsTemp?.filter(
											f =>
												f?.isTagged?.id ===
												selectedDocumentTypes?.[2]?.id
										).length > 0;
									...
									if (
										cacheDocumentsTemp?.filter(f => !f?.isTagged?.id)
											.length > 0
									) {
										isInActiveAddressProofUpload = true;
										// isProceedDisabledAddressProof = true;
								...
								return (
									...
									>
										...
										{sub_section.id.includes(
											...
											>
												<h4>
													Select any one of the documents mentioned
													below for{' '}
													<strong>
														{sub_section?.name
															? 'Permanent'
												...
												<h4>
													{sub_section?.name ? null : (
														<>
															...
															/>
															<label
																htmlFor={CONST.CHECKBOX_SAME_AS_ID}
															>
										...
										<UI_SECTIONS.FormWrapGrid>
											{sub_section?.fields?.map((field, fieldIndex) => {
												...
												if (
													...
													if (
														CONST.HIDE_PRESENT_ADDRESS_FIELDS.includes(
															field.name
														)
													)
														return null;
												...
												const isIdProofUploadField =
													field.type === 'file' &&
													field.name.includes(
														CONST.ID_PROOF_UPLOAD_FIELD_NAME
													);
												...
												if (isSectionCompleted) {
													if (
														sub_section?.id?.includes('address_details')
													) {
														customFieldProps.disabled = false;
													} else {
														customFieldProps.disabled = true;
