module.exports = {
  ...
  friendlyName: 'Validate emudra config',
  ...
  fn: async function (inputs, exits) {
    ...
    try {
      const {userId, loanId, emudraConfig} = inputs;
      ...
      const allowedUsers = emudraConfig.allowedUsers;
      ...
      if (!whiteLabelId || !emudraConfig.allowedWhiteLabelIds.includes(parseInt(whiteLabelId))) {

        throw new Error("White Label Id Not Allowed");

      }

      if (!loanRequestData?.loan_product_id || !emudraConfig.allowedProducts.includes(loanRequestData?.loan_product_id)) {
      ...
      if (!emudraConfig.allowedStages.includes(`${status1} ${status2} ${status3} ${status4}`))

        throw new Error("The current stage is not allowed for EMudra");

      //later check the sub-statuses of the Sanction/Disbursed Stage if it is "In-Principle Sanction" or not

      if (emudraConfig.sanctionDisbursedStage == `${status1} ${status2} ${status3} ${status4}`) {
