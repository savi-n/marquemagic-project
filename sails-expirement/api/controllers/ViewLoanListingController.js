...
// search connection
const redis_conn_d = new redis.Redis({
	host: sails.config.search_redis.host,
	port: 6379
}
);
...
/*}catch (e) {
const redis_conn_d = new redis.Redis({
		host: sails.config.search_redis.host_sec,
		port: 6379
}
);
...
/*const redis_conn_d = new redis.Cluster([{
		host: sails.config.search_redis.host,
		port: 6379
},
{
...
{
		host: sails.config.search_redis.host_third,
		port: 6379
}
]
); */
...
//const redis_conn_d=new redis.Cluster([{host:"ncportal-sampe-001.0pjj1w.0001.aps1.cache.amazonaws.com",port:6379},{host:"ncportal-sampe-002.0pjj1w.0001.aps1.cache.amazonaws.com",port:6379}]);

module.exports = {
	index: async (req, res) => {
		if (!req.param("status1") && !req.param("status2") && !req.param("status3") && !req.param("status4") && !req.param("search") && !req.param("status")) {
		...
		console.log("sjjjjjsssss", sails.config.search_redis.host);
		...
		const loanReq1 = /^[A-Za-z]{4}\d{8}$/;
		const loanReq = /^[A-Za-z]{4}\d{1,8}$/;
		const loanReq2 = /^[A-Za-z]{1}\d{11}$/;
		const email =
		...
		let user_data = req.user,
			...
			ncStatusCondition = {white_label_id: users_whitelabel}, other_users_status;
		...
		if (search && typeof search !== "undefined") {
			let regionList = [], userid = [];
			whereCondition = {white_label_id: users_whitelabel};
			...
			if (
				...
				if (user_data.is_lender_admin === 1) {
					...
					whereCondition.or = [{region_id: regionList}, {users: userid}]; //[{region_id: regionList}, {sales_id: user_data.id}, {createdUserId: user_data.id}, {bank_emp_id: user_data.id}, {assigned_extended_ids: {contains: user_data.id}}];
				...
				console.log("1");
				console.log("---------------------------------whereCondition.orwhereCondition.orwhereCondition.orwhereCondition.or", JSON.stringify(whereCondition));
				if (loanReq1.test(search) === true || loanReq2.test(search) === true) {
					console.log("here");
					let vRedis = null; // await getSearchValue('loans_1' + search, 'loanId');

					if (vRedis == null || vRedis == undefined || vRedis.length == 0) {
						whereCondition.loan_ref_id = {contains: search};
					} else {
						whereCondition.id = vRedis;
					}
				}
				else if (loanReq.test(search) === true) {
					console.log("here2");
					let v_arr_loan_id_match = await getSearchValuePattern(search);
					const v_arr_loan_promise = v_arr_loan_id_match.map(async (key) => {
						console.log(key);
						return await getSearchValue(key, "loanId");
					});
					const v_arr_loan_id = null; // await Promise.all(v_arr_loan_promise);

					console.log(v_arr_loan_id);
					if (v_arr_loan_id == null || v_arr_loan_id == undefined || v_arr_loan_id.length == 0) {
						whereCondition.loan_ref_id = {contains: search};
					} else {
						whereCondition.id = v_arr_loan_id;
					}
					...
					let zReidsKeyBusiness = null; // await iterateOverKeysWithPatternA(search + "*");
					if (zReidsKeyBusiness == null || zReidsKeyBusiness == undefined) {
						whereCondition.businessname = {contains: search};
					} else {
						let arr_loan_id = [];
						for (const i in zReidsKeyBusiness) {
							console.log(zReidsKeyBusiness[i]);
							let v1 = await getSearchValue(zReidsKeyBusiness[i], "loanId");
							arr_loan_id.push(v1);
						}
						if (arr_loan_id.length == 0) {
							whereCondition.businessname = {contains: search};
						} else {
							whereCondition.id = arr_loan_id;
						}
					}
				}
			...
			const result = await sails.helpers.viewloanCondition(req.user, reqData);
			whereCondition = result.whereCondition;
			console.log("++++++++++++++++++++++++++++++++++++++++++++++++", JSON.stringify(whereCondition))
			other_users_status = result.other_users_status;
		}


		console.log("search data", whereCondition);
	...
	getloanlist: async function (req, res, next) {
		// return res.ok({status: "ok", loan_details: [], status_details: []});
		let otherUsers = false;
		if (sails.config.otherUsersTypes.find((element) => element == req.user.usertype)) {
			otherUsers = true;
		}
		let forRes;
		let viewloanCondition = {
		...
		let whereCondition = {};
		const page_count = req.param("skip") ? req.param("skip") : 0,
			limit_count = req.param("limit") ? req.param("limit") : 5,
			search = req.param("search"),
		...
		if (search) {
			viewloanCondition = {
				or: [{businessname: {contains: search}},
			...
			if (req.user.usertype === "Bank") {
				if (req.user.is_lender_admin == 1) {
					const regionList = [],
						regionIds = await LenderRegionMappingRd.find({user_id: req.user.id}).select("region_id");
				...
				viewloanCondition = {users: userids};
			}
		}
		const viewLoanList = await ViewloanRd.find(viewloanCondition).select("id").limit(5);
		_.each(viewLoanList, (value) => {
			loanIds.push(value.id);
		});
		whereCondition = {id: loanIds};
		...
		if (loans.length > 0) {
			Promise.all(
				loans.map(async (loan) => {
					...
					loan.loan_bank_mapping = (await LoanBankMappingRd.find({loan_id: loan.id}).sort("updated_at DESC")) || null;
				})
			).then(() => {
				return res.ok({
					status: "ok",
			...
			if (comments && loanData) {
				...
				if (loanBankMappingData.length > 0) {
					const loanComments = await LoanStatusComments.create({
						...
						status: 1,
						assignee_id: notifyUserId ? notifyUserId : 0,
						comment_ref_id
					}).fetch();
				} else {
					let remarks = {},
	...
	reassignToNc: async (req, res) => {
		...
		let oldObj, status, data, loan_status_id, loan_sub_status_id, update_loanbank;

		const ncstatus_condition = {
		...
		if (loanId && !loanBankId) {
			const taskUserData = await TaskUserMappingRd.findOne({
				...
				taskMaster = await TaskMasterRd.findOne({task_cat_id: 13, white_label_id: loanData.white_label_id, usertype: 'BPO'}).select("usertype");
			if (!taskUserData || !taskMaster) {
				return res.send({
					status: "nok",
					message: 'Unable to send back to CO'
				});
			}
			status = "Sent Back by BPO";
			loan_status_id = 16;
			loan_sub_status_id = 16;
			let taskCommentAdded = await TaskComments.create({
				task_id: taskUserData.id,
				commenter_id: req.user.id,
				comment: comments || "NA",
				created_time: dateTime
			}).fetch();

			await TaskUserMapping.update({id: taskUserData.id})
				.set({
					status: "sent back",
					updated_time: dateTime,
				});
			const users = await UsersRd.findOne({id: taskUserData.assign_userid}).select("name");
			data = {
				userid: taskUserData.assign_userid,
				loan_status_id: 16,
				loan_sub_status_id: 16,
				type: "Case Sent Back to CO",
				message: comments || "NA",
				assignedBy: taskUserData.assign_userid,
				assignedTo: taskUserData.creator_id,
				sent_back_by: users.name,
			};

		} else {
			loan_status_id = 2;
			loan_sub_status_id = 9;
			status = "Reassign to NC";
			if (!loanBankId) {
				return res.badRequest({
					status: "nok",
					message: "Loanbank mapping id is missing."
				})
			}
			await LoanBankMappingRd.findOne({
				...
				if (!loanBankMap) {
					return res.badRequest({
						status: "nok",
						message: "Invalid loanBank mapping id."
					});
				}
				ncstatus_condition.status3 = loanBankMap.loan_bank_status,
					ncstatus_condition.status4 = loanBankMap.loan_borrower_status
				...
				data = reassign_nc_comments = {
					...
					reassigned_user_id: req.user.id,
					comments: comments,
					userId: req.user.id,
					assigned_by: req.user.name,
					message: comments
		...
		ncStatusData = await NcStatusManageRd.find(ncstatus_condition).select("name");
		...
		if (!loanData.nc_status_history) {
			...
			let jsonNcStatus = JSON.parse(loanData.nc_status_history);
			jsonNcStatus[dateTime] = data;
			nc_status = JSON.stringify(jsonNcStatus);
		}
		const update_loandata = await Loanrequest.updateOne({
			id: loanId
		}).set({
			loan_status_id,
			loan_sub_status_id,
		...
		if (update_loandata) {
			return res.ok({
				status: "ok",
				message: sails.config.msgConstants.successfulUpdation,
				data: {loanData: update_loandata, loanBankMappingData: update_loanbank}
			});
		} else {
			return res.badRequest({
				status: "nok",
	...
	viewComments: async function (req, res) {
		...
		if (isOtherUser) {
			for (key in remarks) {
				const {userId: commentorId, assigneeId, assignedBy, assignedTo} = remarks[key];
				remarks[key].commented_at = moment(key).add({h: 5, m: 42}).format("YYYY-MM-DD HH:mm:ss").toString();
				if (userType == "BPO") {
					if (remarks[key].comment_ref_id) {
						if (assignedBy && typeof (remarks[key].assignedBy) === 'number') {
							let user = await UsersRd.findOne({id: assignedBy});
							remarks[key].assignedBy = user.name;
						}
						if (assignedTo && assignedTo !== 0 && typeof (remarks[key].assignedTo) === 'number') {
							let user = await UsersRd.findOne({id: assignedTo});
							remarks[key].assignedTo = user && user.name ? user.name : null;
						}
						remarksArr.push(remarks[key]);
					}
				}
				else {
					if (userId == commentorId || userId == assigneeId || userId == assignedBy || userId == assignedTo) {
						if (assignedBy && typeof (remarks[key].assignedBy) === 'number') {
							let user = await UsersRd.findOne({id: assignedBy});
							remarks[key].assignedBy = user.name;
						}
			...
			for (key in remarks) {
				remarks[key].commented_at = moment(key).add({h: 5, m: 42}).format("YYYY-MM-DD HH:mm:ss").toString();
				if (remarks[key].assignedBy && typeof (remarks[key].assignedBy) === 'number') {
					let user = await UsersRd.findOne({id: remarks[key].assignedBy});
					remarks[key].assignedBy = user.name;
				}
				if (remarks[key].assignedTo && typeof (remarks[key].assignedTo) === 'number') {
					let user = await UsersRd.findOne({id: remarks[key].assignedTo});
					remarks[key].assignedTo = user.name;
				}
				remarksArr.push(remarks[key]);
			}
		}
		...
		if (loanBankMappingInputs.length > 0) {
			...
			if (loanComments.length > 0) {
				for (const element of loanComments) {
					...
					if (isOtherUser === 1 && userType === "BPO") {  //LOAN STATUS COMMENTS
						user_id = await UsersRd.findOne({id: element.user_id}).select("usertype");
						assignee_id = await UsersRd.findOne({id: element.assignee_id}).select("usertype");
						if ((element.user_id && user_id.usertype === "BPO") || (element.assignee_id && assignee_id.usertype === "BPO") || element.user_type === "BPO") {
							loanBankMappingComments.push(comments);
						}
					} else if (isOtherUser === 1 && userType !== "BPO") {
		...
		if (viewCommentHistory.rows.length > 0) {
			...
			for (task_data of viewData) {
				...
				if (userType === "BPO") {
					if (!task_data.commenter_id && !task_data.created_time && !task_data.other_user_comment) {
						continue;
					}
					let commenter = await UsersRd.findOne({id: task_data.commenter_id}).select("name");
					legalObj.assignedBy = legalObj.assigned_by = commenter.name;
					let assign_to = await UsersRd.findOne({id: task_data.assign_userid}).select("name");
					legalObj.assignedTo = assign_to.name;
				}
		...
		if (userType == "BPO") {
			let key = "comment_ref_id";
			remarksArr = remarksArr.filter(obj1 => !formatedloanBankMappingComments.some(obj2 => obj1[key] === obj2[key]));
		}
	...
	creditUserListNew: async function (req, res) {
		const {branch_id, city, loggedInWhiteLabelID} = req.user,
			whereCondition = {
				white_label_id: {contains: loggedInWhiteLabelID}
			},
...
async function loanDetailsForView(reqData, search) {
	try {
		...
		const [loan_product_details, parent_product, dbdocData] = await Promise.all([
		...
		const dataObj = {
			...
			comments_reply_success: await comments_notification(loan_bank_mapping, loanData.remarks),
			// cibil_pulled_for_applicant_coApplicant: cibilFlag(director_details)
			cibil_pulled_for_applicant_coApplicant: true
		};
		if (loan_bank_mapping) {
			dataObj.loan_bank_mapping = await LoanBankMappingRd.findOne({id: loan_bank_mapping}).select(["bank_id", "bank_emp_id", "loan_bank_status", "loan_borrower_status", "meeting_flag", "assigned_extended_ids", "lender_status", "offer_additional_data", "reassign_nc_comments"]);
...
async function iterateOverKeysWithPatternA(parm) {
	const keys = [];
	...
	do {
		// Scan keys starting from the current cursor position with the specified pattern
		const [nextCursor, matchingKeys] = await redis_conn_d.scan(cursor, 'MATCH', 'busi_1:' + parm);
		// Update cursor for next iteration
		cursor = nextCursor;
		//console.log(matchingKeys);
		// Add matching keys to the list
		keys.push(...matchingKeys);
	} while (cursor !== '0');

	return keys;
}
async function getSearchValuePattern(hashkey) {
	const keys = [];
	let cursor = '0';
	do {
		const [nextCursor, MatchingKeys] = await redis_conn_d.scan(cursor, 'MATCH', 'loans_1' + hashkey + '*');
		cursor = nextCursor;
		keys.push(...MatchingKeys);
	} while (cursor !== '0');
	return keys;
...
async function hgetWithLikeSearch(pattern, field) {
	let cursor = '0';
	const values = [];

	do {
		const [nextCursor, keys] = await redis_conn_d.scan(cursor, 'MATCH', pattern);
		cursor = nextCursor;

		for (const key of keys) {
			const value = await redis_conn_d.hget(key, field);
...
async function getSearchValue(hashKey, field) {

	try {
		const value = await redis_conn_d.hget(hashKey, field);
		console.log(`Value of field '${field}' in hash '${hashKey}':`, value);
		return value;
	} catch (error) {
...
async function directorDetails(businessid) {
	...
	directorDetailsData = await DirectorRd.find({
		business: businessid,
		status: "active"
	}).select(["dpancard", "isApplicant", "daadhaar", "dpassport", "dvoterid", "dfirstname", "dlastname", "demail", "dcontact", "middle_name", "ddlNumber", "income_type", "type_name", "dcibil_score"]).limit(20);
...
async function comments_notification(loan_bank_mapping_id, remarks) {
	let commentFlag = true;
	const arrayData = [];
	if (remarks) {
		const parseRemarks = JSON.parse(remarks);
		arrayData.push(Object.values(parseRemarks));
	}
	if (loan_bank_mapping_id) {
		loanComments = await LoanStatusCommentsRd.find({loan_bank_id: loan_bank_mapping_id}).sort("id DESC");
		arrayData.push(loanComments);
	}
	const flattenedArrayData = arrayData.flat(),
		count = countRecordsByCommentRefId(flattenedArrayData);
	if ((!count.undefined || count.undefined > 1) && Object.values(count).includes(1)) {
		commentFlag = false;
	}
	return commentFlag;

}
const countRecordsByCommentRefId = (data) => {
	const counts = data.reduce((acc, item) => {
		console.log(item.comment_ref_id);
		const refId = item.comment_ref_id || 'undefined';
...
const cibilFlag = (directorData) => {
	return !directorData.some(cibil => cibil.dcibil_score === 0);
};
